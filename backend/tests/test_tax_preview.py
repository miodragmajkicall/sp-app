...
def _ensure_tenant(db, tenant_code: str) -> None:
    ...
    for col in tenants.columns:
        if col.name == "id":
            # PK je string pa možemo staviti bilo koju jedinstvenu vrijednost
            row[col.name] = "tenant-tax-auto"
        elif col.name == "code":
            row[col.name] = tenant_code
        elif col.name == "name":
            row[col.name] = "Tenant za tax auto test"
        elif (
            not col.nullable
...
def test_tax_monthly_preview_formula_basic() -> None:
    """
    Osnovni test za /tax/monthly/preview:
    - unese ručno total_income i total_expense
    - provjeri da backend primjenjuje DUMMY formulu
    """
    tenant_code = "t-preview-tenant"
